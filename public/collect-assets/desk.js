/* 影视采集站信号台 —— 纯静态，无后端，数据来自 catalog.json */
const $ = (s, r = document) => r.querySelector(s);
const $$ = (s, r = document) => [...r.querySelectorAll(s)];

const SITE_BASE = (document.body?.dataset?.base || "/").replace(/\/?$/, "/");
const DESK_ROOT = `${SITE_BASE}collect/desk/`;
const ASSET_ROOT = `${SITE_BASE}collect-assets/`;
const BLOG_HOME = document.body?.dataset?.blogHome || SITE_BASE;
const PAGE_MAP = {
  "index.html": DESK_ROOT,
  "category.html": `${DESK_ROOT}category/`,
  "detail.html": `${DESK_ROOT}detail/`,
  "download.html": `${DESK_ROOT}download/`,
  "guide.html": `${DESK_ROOT}guide/`,
  "submit.html": `${DESK_ROOT}submit/`,
};
const pageUrl = (name) => PAGE_MAP[name] || `${DESK_ROOT}${name}`;
const catalogUrl = () => `${ASSET_ROOT}catalog.json`;
...
function showBootError(err) {
  ...
  box.style.cssText = 'position:fixed;inset:auto 16px 16px 16px;z-index:9999;background:#3b0d12;color:#ffe8ea;padding:14px 16px;font:13px/1.5 Menlo,Consolas,monospace;border:1px solid #ff8f96';
  box.innerHTML = `<b>信号台启动失败</b><br>${msg}<br>请强制刷新（Ctrl+F5），或打开 <a href="${catalogUrl()}" style="color:#ffd0d4">catalog.json</a> 检查数据是否可访问。`;
  document.body.appendChild(box);
  console.error('[signaldesk]', err);
}
...
async function bootInner() {
  const res = await fetch(`${catalogUrl()}?t=${Date.now()}`, { cache: 'no-store' });
  if (!res.ok) throw new Error(`catalog.json HTTP ${res.status}`);
  const DATA = await res.json();
  if (!DATA || !Array.isArray(DATA.apis)) throw new Error('catalog.json 格式异常：缺少 apis');
  ...
  $('#hdr').innerHTML = `
    ...
    <div class="container mast-brand">
      <div>
        <a class="brand-name" href="${pageUrl('index.html')}">${DATA.brand}</a>
        <div class="brand-sub">CaiJiZhan Signal Desk — 每日巡检 · 公开透明 · 网友共建</div>
      </div>
      <nav class="main-nav">
        <a href="${BLOG_HOME}">返回博客</a>
        <a href="${pageUrl('index.html')}" data-p="index">首页</a>
        <a href="${pageUrl('category.html')}" data-p="category">分类</a>
        <a href="${pageUrl('download.html')}" data-p="download">一键配置</a>
        <a href="${pageUrl('guide.html')}" data-p="guide">对接教程</a>
        <a href="${pageUrl('submit.html')}" data-p="submit">投稿</a>
      </nav>
    </div>`;
  ...
  <div class="container foot-grid">
    <div class="foot-brand">
      <a class="foot-name serif" href="${pageUrl('index.html')}">${DATA.brand}</a>
      <p>每日巡检网友投稿的采集接口，如实刊出谁在岗、谁告病、谁失联。本站无后端，数据全部存于 <code>catalog.json</code>。</p>
      <p class="foot-fine">接口均来自网友投稿，请自行甄别合规性与授权范围。</p>
    </div>
    <div class="foot-col">
      <h5>站内导航</h5>
      <a href="${pageUrl('index.html')}">信号目录</a>
      <a href="${pageUrl('category.html')}">内容分类</a>
      <a href="${pageUrl('download.html')}">一键配置下载</a>
      <a href="${pageUrl('guide.html')}">对接教程</a>
      <a href="${pageUrl('submit.html')}">投稿接口</a>
    </div>
    <div class="foot-col">
      <h5>联系与共建</h5>
  ...
  document.documentElement.dataset.deskReady = String(DATA.apis.length);
}
...
function pageIndex(DATA, apis) {
  ...
  $('#top10').innerHTML = top.map((a, i) => `
    <li><span class="no">${i + 1}</span>
      <span class="nm"><a href="${pageUrl('detail.html')}?id=${a.id}">${a.name}</a></span>
  ...
  if (title) {
    const tip = title.querySelector('#cnt');
    title.textContent = '精选接口 ';
  ...
  const rows = () => {
    ...
    $('#tbody').innerHTML = list.length ? list.map(a => `
      <tr>
        <td class="name"><a href="${pageUrl('detail.html')}?id=${a.id}">${a.name}</a>${a.restricted ? ' <span class="st mute"><i></i>受限</span>' : ''}</td>
        <td>${stBadge(a.s)}</td>
...
function drawWave(cv, apis) {
  ...
  cv.addEventListener('click', () => { if (hoverApi) location.href = pageUrl('detail.html') + '?id=' + hoverApi.id; });
  let hoverApi = null;
...
function initRadar(apis) {
  ...
  cv.addEventListener('click', () => { if (hover) location.href = pageUrl('detail.html') + '?id=' + hover.a.id; });
    ...
    <section class="cat-sec" id="cat-${encodeURIComponent(c)}">
      ...
      <table class="t">
        ...
        <tbody>${list.sort((x, y) => (x.s.avg || 9999) - (y.s.avg || 9999)).map(a => `
          <tr>
            <td class="name"><a href="${pageUrl('detail.html')}?id=${a.id}">${a.name}</a></td>
            <td>${stBadge(a.s)}</td>
            <td class="mono">${a.s.lastMs ? a.s.lastMs + 'ms' : '—'}</td>
            <td class="mono">${(a.s.up * 100).toFixed(1)}%</td>
            <td>${a.contributor}</td>
...
function pageDetail(DATA) {
  ...
  if (!a) { $('#main').innerHTML = `<div class="container prose"><div class="empty">未找到该接口，<a href="${pageUrl('index.html')}">返回目录</a></div></div>`; return; }
  const s = statOf(a);
  ...
  $('#main').innerHTML = `
  <div class="container">
    <div class="crumb"><a href="${pageUrl('index.html')}">信号目录</a> / 采集站详情</div>
    ...
    <div class="d-section grid2">
      <div class="box">
        ...
        <div style="font-size:12.5px;margin-top:6px"><a href="${pageUrl('guide.html')}" style="border-bottom:1px solid var(--ink2)">查看采集对接教程 →</a></div>
      </div>
      <div class="box">
